...
class NVActivityIndicatorAnimationBallSpinFadeLoader: NVActivityIndicatorAnimationDelegate {
    ...
    func setUpAnimation(in layer: CALayer, size: CGSize, color: UIColor) {
        ...
        // Draw circles
        for i in 0 ..< 8 {
            let circle = circleAt(CGFloat(M_PI_4 * Double(i)),
                                  size: circleSize,
    ...
    func circleAt(_ angle: CGFloat, size: CGFloat, origin: CGPoint, containerSize: CGSize, color: UIColor) -> CALayer {
        let radius = containerSize.width / 2 - size / 2
        let circle = NVActivityIndicatorShape.circle.layerWith(CGSize(width: size, height: size), color: color)
        let frame = CGRect(
