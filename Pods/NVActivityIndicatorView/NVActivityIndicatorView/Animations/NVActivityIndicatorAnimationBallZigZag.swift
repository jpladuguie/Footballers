...
class NVActivityIndicatorAnimationBallZigZag: NVActivityIndicatorAnimationDelegate {
    ...
    func setUpAnimation(in layer: CALayer, size: CGSize, color: UIColor) {
        ...
        // Draw circle 1
        circleAt(frame, layer: layer, size: CGSize(width: circleSize, height: circleSize), color: color, animation: animation)
    ...
    func circleAt(_ frame: CGRect, layer: CALayer, size: CGSize, color: UIColor, animation: CAAnimation) {
        let circle = NVActivityIndicatorShape.circle.layerWith(size, color: color)
